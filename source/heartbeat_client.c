...
// V2: raw socket HTTP/1.0, Bearer token 认证, 最小 JSON 解析
...
static int http_connect(const char *host, int port) {
    ...
    /* 设置连接超时 */
    struct timeval tv;
    tv.tv_sec = TUNNEL_CONNECT_TIMEOUT_SEC;
    tv.tv_usec = 0;
    ...
    /* 设置接收超时 */
    tv.tv_sec = TUNNEL_RECV_TIMEOUT_SEC;
    tv.tv_usec = 0;
    setsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv));
...
static bool http_post_json(const char *host, int port, const char *path,
...
static void heartbeat_thread_func(void *arg) {
    ...
    while (s_running) {
        ...
        /* 发送心跳 */
        bool ok = http_post_json(TUNNEL_SERVER_HOST, TUNNEL_SERVER_PORT,
                                 TUNNEL_HEARTBEAT_PATH, TUNNEL_PSK,
